...
export const loginSchema = z.object({
    ...
    password: z.string({required_error:"Password cant be empty"})
...
export const listingSchema = z.object({
    ...
    category: z.enum(["room","vehicle","service"],{
    errorMap: () => ({ message: "Category must be room, vehicle or service" }),
  })
                .optional(),

    price : z.number({required_error:"Price must be included", 
                        invalid_type_error:"Price must be a number"})
            .positive("Price must be greater than 0"),

    price_unit: z.enum(['per_hour','per_day','per_month','per_year'],{
        errorMap: ()=>({message: "Price unit must be per_hour, per_day, per_month, per_year"})
    }),
...
export const createBookingSchema = z.object({
        listing_id: z.string({required_error:"Listing id is required"})
        ...
        path:['end-date']
}).refine(date => new Date(data.start_date) >= new Date().setHours(0,0,0,0),{
...
export const verificationSchema = z.object({
        ...
        phone: z.string({required_error:"Full name is required"})
                   .regex(/^(98|97)\d{8}$/, "Phone must be a valid Nepal number (98XXXXXXXX)"),
        address: z.string({required_error:"Address is required"})
                   .min(5,"Please enter your full address"),

        citizenship_img: z.string({required_error:"Citizenship image is required"})
